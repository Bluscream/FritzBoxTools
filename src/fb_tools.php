...
if (!isset($cfg)) { // $cfg schon gesetzt?
  $cfg = array(
    ...
    // Fritz!Box Kennwort
    'user' => false,
    // Fritz!Box Username (Optional)
    'port' => 80,
    // Fritz!Box HTTP-Port (Normalerweise immer 80)
    'fiwa' => 100,
    // Fritz!Box Firmware (Nur Intern)
    'upnp' => 49000,
    // Fritz!Box UPnP-Port (Normalerweise immer 49000)
    'pcre' => 64 * 1024 * 1024,
    // pcre.backtrack_limit
    'sbuf' => 4096,
    // TCP/IP Socket-Buffergrφίe
    'tout' => 30,
    // TCP/IP Socket-Timeout
    'upda' => 60 * 60 * 24 * 100,
    // Auto-Update-Check Periode (Kein Update-Check: 0)
    'wrap' => 'auto',
    // Manueller Wortumbruch (Kein Umbruch: 0)
    'char' => 'auto',
    // Zeichenkodierung der Console (auto/ansi/oem/utf8)
    'dbfn' => 'debug#.txt',
    // Template fόr Debug-Dateien
    'time' => 'Europe/Berlin',
    // Zeitzone festlegen
    'drag' => 'konfig fcs,-d',
    // Drag'n'Drop-Modus
    ...
    // Ausgaben speichern
    'zlib' => array('mode' => -1),
    // ZLib-Funktionen (mode: packlevel)
...
if (!function_exists('file_put_contents')) { // http://php.net/file_put_contents
  function file_put_contents($file, $data, $opt = 0)
  { // $opt ist nicht vollstδndig implemmentiert
...
if (!function_exists('gzdecode')) { // http://php.net/gzdecode (Workaround)
  function gzdecode($data)
  {
    ...
    if ($tmp = tempnam(null, 'gz') and $fp = fopen($tmp, 'w')) { // Gepackte Daten speichern
      ...
      @unlink($tmp); // άberreste lφschen
    }
    return $data;
  }
}
function file_read($file)
{ // (Gepackte) Datei lesen
  global $cfg;
...
function crc_32($str, $file = false)
{ // Berechnet die CRC32 Checksume von einen String (Optional von einer Datei)
  return str_pad(sprintf('%X', crc32(($file) ? file_get_contents($str) : $str)), 8, 0, STR_PAD_LEFT);
}
function ifset(&$x, $y = false)
{ // Variabeln prόfen und Optional vergleichen (var,test)
...
function preg_array($x, $y, $z = 0, $w = array())
{ // Durchsucht einen Array mit Regulδren Ausdruck (preg,array,mode)
...
function out($str, $mode = 0)
{ // Textconvertierung vor der ausgabe (mode: 0 -> echo / 1 -> noautolf / 2 -> debug)
  global $cfg;
  if ($str) {
    if (is_array($str))
    ...
    if (!($mode / (1 << 1) % 2) and preg_match('/\S$/D', $str)) // AutoLF
      $str .= "\n";
    if ($mode / (1 << 2) % 2) // Unnφtige Whitespaces im Debug-Modus lφschen
      $str = preg_replace('/(?<=\n\n|\r\n\r\n)\s+$/', '', $str);
    ...
    elseif (ifset($cfg['char'], '/utf8|c(odepage|p)65001/'))
      $str = utf8_encode($str);
    elseif ($cfg['char'] == 'html' and preg_match_all('/(.)([\da-z#]+)/', '&amp<lt>gt"quot\'#39δaumlφoumlόuumlίszligΔAumlΦOumlάUuml', $m))
      $str = str_replace($m[1], preg_replace('/.+/', '&$0;', $m[2]), $str);
    elseif (
      ifset($cfg['char'],
        '/^\d+$/') and dechex($cfg['char']) == 539
      and preg_match_all('/([a-z\d])(.)|(.)(..)/i',
        '1I2Z3E4A5S6G7T8B9g0Oa4A4b8B8c<C(e3E3g6G6h#H#i!I!l1L1o0O0q9Q9s5S5t7T7x+X+z2Z2δ4:Δ4:φ0:Φ0:όu:άU:ί55§55',
        $m)
      and preg_match_all('/(.)(.+)/',
        implode("\n",
          $m[0]),
    ...
    elseif (!ifset($cfg['char'], '/^(ansi|(codepage|cp)1252|iso.?8859.?1|ascii)$/i') and preg_match_all('/(.)(..)/', 'δaeφoeόueίssΔAeΦOeάUe§SS', $m))
      $str = str_replace($m[1], $m[2], $str);
    if ((int) $cfg['wrap'] and ifset($cfg['char'], '/7bit|(13{2}|utf)7/')) // Wortumbruch
      $str = wordwrap($str, $cfg['wrap'] - 1, "\n", true);
  }
...
function dbug($str, $level = 0, $mode = 4)
{ // Debug-Daten ausgeben/speichern (mode: 3 -> NoTime)
  global $cfg;
...
function errmsg($msg, $name = 'main')
{ // Fehlermeldung(en) Sichern
  global $cfg;
  if ($msg) { // Fehlermeldung speichern
    ...
    while (isset($cfg['error'][$name]) and is_array($cfg['error'][$name])) // Fehlermeldung vorhanden?
      if ($val = end($cfg['error'][$name]) and preg_match('/^\w+$/', $val)) // Mφglicher Rekusive Fehlermeldung?
        $name = $val; // Nδchste Fehlermeldung suchen
      else
        return $val; // Fehlermeldung ausgeben
  }
  return false; // Funktion fehlgeschlagen
}
function makedir($dir, $mode = 1)
{ // Erstellt ein Verzeichnis und wechselt dorthin
  if (!$dir or $dir == '.') // Self-Dir nicht bearbeiten
    return true;
  $dir = preg_replace('/[\\\\\/]$/', '', $dir); // Abschlussshlash entfernen
  if (strpos($dir, '%') !== false) // strftime auflφsen (Problematische Zeichen werden umgewandelt)
    $dir = strftime($dir);
  if (preg_match('/^(\w:)?(.*)$/', $dir, $var))
    $dir = $var[1] . preg_replace('/[<:*|?">]+/', '-', $var[2]);
  if (!file_exists($dir)) { // Neues Verzeichniss erstellen
    if ($mode) // Debug-Meldung unterdrόcken
...
{ // Konvertiert eine Einfache XML-Datei (Ohne Attribute) in ein Array
...
function tar2array($file)
{ // Liest ein Tar-Archiv als Array ein
  global $cfg;
  ...
  return errmsg("TAR-Archiv nicht gefunden oder lδίt sich nicht φffnen", __FUNCTION__);
}
function data2tar($name, $data = '', $time = 0)
{ // Erstellt ein Tar-Header
...
function request($method, $page = '/', $body = false, $head = false, $host = false, $port = false)
{ // HTTP-Request durchfόhren
  global $cfg;
...
//  stream_set_blocking($fp,0);
    if ($method == 'POST') { // POST-Request vorbereiten
      ...
      $body = "\r\n";
    if (!isset($head['host'])) // Host zum Header hinzufόgen
      $head['host'] = $host;
    if (!isset($head['connection'])) // Connection zum Header hinzufόgen
    ...
    if ($mode == 'putonly') // Nur Upload durchfόhren
      return fclose($fp);
          ...
          if ($rp = substr($rp, $pos + 4) or !feof($fp)) { // Nur weitermachen wenn noch Daten kommen
            ...
            if ($sp = ($gz) ? $cfg['zlib']['open']($file[1], 'w' . $cfg['zlib']['mode']) : fopen($file[1], 'w')) {
              ...
              if ($gz)
                ...
                fclose($sp);
              dbug("\n", 0, 8); // Download-Anzeige abschlieίen
              $rp = $header;
              ...
              return errmsg("$file[1] kann nicht zum Schreiben geφffnet werden", __FUNCTION__);
          } else
    ...
    fclose($fp);
    dbug("\n", 6, 8); // Download-Anzeige abschlieίen
    $fp = $rp;
...
function response($xml, $pass, $page = false)
...
function login($pass = false, $user = false, $sid = 0)
{ // In der Fritz!Box einloggen
  global $cfg;
  ...
  $preg = '/<SID>(\w+)<\/SID>/i';
  if ($sid) { // SID-Sessioen bestδtigen lassen
...
//   $err = ($var = errmsg(0,'request')) ? ", $var" : ", keine Antwort";
  ...
  return ($cfg['sid'] = $sid) ? $sid : errmsg("Anmeldung fehlgeschlagen$err" . ((!ifset($pass, '/^[ -~]+$/')) ? 
    "\nHinweis: Das Login-Kennwort enthδlt Sonderzeichen, die bei unterschiedlicher Zeichenkodierung Probleme bereiten kφnnen" : ""), __FUNCTION__);
}
function logout($sid = 0)
{ // Aus der Fritz!Box ausloggen
  if (!$sid)
    $sid = $GLOBALS['cfg']['sid'];
...
{ // Supportcode aufschlόsseln
  dbug("Enschlόssle Supportcode");
  return ($str or $str = request('GET', '/cgi-bin/system_status')) ? ((preg_match('!
    ...
    . "fw_attrib: " . (($array[11] < 64) ? "Modifiziert" : "Unverδndert") . "\n\n"
  ...
  if (
    $rp = request(
      array(
        ...
        'head' => array_merge($GLOBALS['cfg']['head'],
          array('content-type' => 'text/xml; charset="utf-8"', 'soapaction' => "\"$ns#$rq\"")),
...
{ // Externe IPv4-Adresse όber UPnP ermitteln
  global $cfg;
  dbug("Ermittle IP-Adresse όber UPnP");
  $ip = array();
...
{ // Internetverbindungen όber UPnP neu aufbauen
  dbug("WAN Neueinwahl όber UPnP");
...
{ // HTTP-Downloads in Datei speichern
...
function supportdata($file = false, $tm = false, $sid = 0)
{ // Supportdaten anfordern
...
function supportdataextrakt($data, $mode = 0, $file = '')
{ // Supportdaten extrahieren
  global $cfg;
  $info = array();
  if (is_array($mode) and $mode[0] == 'sec') {
    ...
    foreach ($array as $key => $var) { // Maximale Lδngen ermitteln
      if ($mode == 2 and $fp)
    ...
    if ($mode and $fp) { // Tar-Archiv abschlieίen
...
function dial($dial, $port = false, $sid = 0)
{ // Wahlhilfe
  if (!$sid)
    $sid = $GLOBALS['cfg']['sid'];
  ...
  if ($GLOBALS['cfg']['fiwa'] >= 530) {
    ...
    dbug("Dial: " . (($rdial) ? "Wδhle $dial" . (($port) ? " fόr Telefon $port" : "") : "Auflegen"));
  }
  return true;
}
function cfgexport($mode, $pass = false, $sid = 0)
{ // Konfiguration Exportieren (NUR Exportieren)
...
{ // Checksumme fόr die Konfiguration berechnen
...
function cfgimport($file, $pass = '', $mode = false, $sid = 0)
{ // Konfiguration importieren (Wird vermutlich bald όberarbeitet)
  global $cfg;
  if (
    $file and (is_file($file) and preg_match($cfg['ptar'],
      $file) and ($data = cfgmake(tar2array($file)))
      or is_file($file) and ($data = file_read($file))
      or is_dir($file) and ($data = cfgmake($file))
    ) or !$file and $data = $mode and substr($mode,
      0,
      4) == '****'
  ) {
...
function cfginfo($data, $mode = 0, $file = '', $text = false)
{ // Konfiguration in Einzeldateien sichern (mode: 0->show, 1->Dir, 2->Tar, 3->tgz)
  global $cfg;
  if (
    preg_match_all('/^(?:
	\*{4}\s(.*?)\sCONFIGURATION\sEXPORT|(\w+=\S+))\s*$		# 1 Fritzbox-Modell, 2 Variablen
	|^\*{4}\s(?:CRYPTED)?(CFG|BIN)FILE:(\S+)\s*?\r?\n(.*?)\r?\n	# 3 Typ, 4 File, 5 Data
	^\*{4}\sEND\sOF\sFILE\s\*{4}\s*?$/msx',
      $data,
      $array) and $array[1][0] and $crc = cfgcalcsum($data)
  ) {
    $list = $val = $vars = array();
    $mstr = $mlen = array(0, 0);
    ...
    foreach ($list as $key => $var) { // Maximale Lδngen ermitteln
...
function cfgmake($dir, $mode = '', $file = false)
{ // Konfiguration wieder zusammensetzen
  if (is_array($dir) and isset($dir[0]) and !$mode and !$file) { // Helper fόr Preg_Replace CfgMake
    ...
    $dir and (is_array($dir) and $tar = preg_array('/^(index|pattern)\.txt$/',
      $dir,
      1) and ($data = $dir[$tar])
      or ($var = glob("$dir/{index,pattern}.txt",
      GLOB_BRACE)) and ($data = file_get_contents($var[0])))
    and preg_match('/^\*{4}\s+\w+.*CONFIGURATION EXPORT/',
      $data,
      $array)
  ) {
...
function konfigdecrypt($data, $pass, $sid = 0)
{ // Konfig-Datei mit Fritz!Box entschlόsseln
  global $cfg;
  ...
  if ($sid and preg_match_all('/^\s*([\w-]+)\s*=\s*("?)(\${4}\w+)\2;?\s*$/m', $data, $array) and preg_match('/(?:\s*Password\d*=\${4}\w+)+/', $data, $var)) {
    if (
      preg_match('/^boxusers\s\{\s*^\s{8}users(\s\{.*?^\s{8}\}$)/smx',
        $k = str_replace("\t",
          "        ",
            $cfg['zlib']['inflate'](base64_decode("
	...
	p9UP/d7AkX4qhISd/yYYcfNkaNT9QPKInKsrZ70C23uKbXVZWtDamq23BvHHyGJdfYw8RvJj5KHDeIx4+J76mBVpqPvhmkgN/6s/msSfIPA/"))),
        $v)
    )
      $export = array(str_replace("#0", $var[0], $k), $v[1]); // Stark gekόrze und leere Fritz!Box 7490 Konfig
    if (preg_match_all('/\${4}\w+/', $var[0], $val)) // Salt-Kennwφrter entfernen
    ...
    if (preg_match_all('/^\*\*\*\*\sBINFILE:(.*)\s*([\dA-F\s]*?(?:24\s*){4}(?:[46][1-9A-F]|[57][0-9A]|3\d|\s*)+[\dA-F\s]*)/mi', $data, $binfile)) {
      $binfile = array_combine($binfile[1], $binfile[2]); // Binfiles mit verschlόsselten Werten einbeziehen
    ...
    $list = array_unique($array[3]); // Doppelte Eintrδge entfernen
    shuffle($list); // Alle Eintrδge durchwόrfeln (Fόr Problemfδlle)
    if (preg_match_all('/\${4}\w+/', $var[0], $array)) // Salt-Kennwφrter eintragen
    ...
    $pregimport = array(
      'de' => array(
        ...
        'Dyn(amic )?DNS' => array(2, 1, '/(?<=Domainname:\s|Benutzername:\s)(.*?)(?=,\s|$)/'),
        'PushService' => array(1, 3, '/^E-Mail-Empfδnger:\s(.*?)$/'),
        'MyFRITZ!' => $dupe[0],
        'FRITZ!Box-Benutzer' => $dupe[1]
      ),
      'en' => array(
        'Internet Account Information' => array(1, 0, '/^User:\s(.*?)(?:,\sProvider:\s.*?)?$/'),
        'Dyn(amic )?DNS' => array(2, 1, '/(?<=Domain\sname:\s|user\sname:\s)(.*?)(?=,\s|$)/'),
        'Push service' => array(1, 3, '/^e-mail\srecipient:\s(.*?)$/'),
        'MyFRITZ!' => $dupe[0],
        'FRITZ!Box Users' => $dupe[1]
      ),
      'es' => array(
        'Datos de acceso a Internet' => array(1, 0, '/^Usuario:\s(.*?)(?:,\sProvider:\s.*?)?$/'),
        '(Dyn)?DNS( dinαmico)?' => array(2, 1, '/(?<=Nombre\sdel\sdominio:\s|nombre\sdel\susuario:\s)(.*?)(?=,\s|$)/'),
        'Push Service|Notificaciones' => array(1, 3, '/^Destinatario\sde\scorreo:\s(.*?)$/'),
        'MyFRITZ!' => $dupe[0],
        'Usuarios de FRITZ!Box' => $dupe[1]
      ),
      'fr' => array(
        'Donnιes d\'accθs ΰ Internet' => array(1, 0, '/^Utilisateur[\xa0\s]?:[\xa0\s](.*?)(?:,\sProvider:\s.*?)?$/'),
        '(Dyn)?DNS( dynamique)?' => array(2, 1, '/(?<=Nom\sde\sdomaine[\xa0\s]:[\xa0\s]|nom\sd\'utilisateur[\xa0\s]:[\xa0\s])(.*?)(?=,\s|$)/'),
        'Service push' => array(1, 3, '/^Destinataire\sdu\scourrier\sιlectronique[\xa0\s]?:[\xa0\s](.*?)$/'),
        'MyFRITZ!' => $dupe[0],
        'Utilisateur de FRITZ!Box' => $dupe[1]
      ),
      'it' => array(
        'Dati di accesso a Internet' => array(1, 0, '/^Utente:\s(.*?)(?:,\sProvider:\s.*?)?$/'),
        'Dyn(amic )?DNS' => array(2, 1, '/(?<=Nome\sdi\sdominio:\s|nome\sutente:\s)(.*?)(?=,\s|$)/'),
        'Servizio Push' => array(1, 3, '/^Destinatario\se-mail:\s(.*?)$/'),
        'MyFRITZ!' => $dupe[0],
        'Utenti FRITZ!Box' => $dupe[1]
      ),
      'pl' => array(
        'Dane dost\?powe do internetu' => array(1, 0, '/^U\?ytkownik:\s(.*?)(?:,\sProvider:\s.*?)?$/'),
        'Dyn(amic )?DNS' => array(2, 1, '/(?<=Nazwa\sdomeny:\s|nazwa\su\?ytkownika:\s)(.*?)(?=,\s|$)/'),
        'Push Service' => array(1, 3, '/^Odbiorca\se-maila:\s(.*?)$/'),
        'MyFRITZ!' => $dupe[0],
        'U\?ytkownicy FRITZ!Box' => $dupe[1]
      ),
    );
    $lang = (ifset($cfg['boxinfo']['Lang']) and ifset($pregimport[$cfg['boxinfo']['Lang']])) ? $cfg['boxinfo']['Lang'] : 'de';
    dbug((count($list) + count($plain)) . " verschiedene verschlόsselte Eintrδge gefunden! (Sprachmuster: $lang)");
    dbug($list, 4, 'KonfigDeCrypt-List');
    $b = 1;
    while ($list) { // Alle Verschlόsselte Eintrδge durchlaufen
      $import = $export[0];
      $buffer = array_values($buffer);
      while ($list and count($buffer) < ((ifset($cfg['boxinfo']['Name'], '/FRITZ!Box 7312/i')) ? 4 : 20)) // Die ersten 20 Eintrδge sichern
        $buffer[] = array_shift($list);
      $a = 0; // Import-Buffer fόllen
      $v = array();
      ...
      if (
        $var = request('POST',
          ...
          array(
            'sid' => $sid,
            'ImportExportPassword' => $pass,
            'ConfigTakeOverImportFile' => array('filename' => 'fritzbox.export', 'Content-Type' => 'application/octet-stream', '' => $import),
            'apply' => false
          )
        ) and !preg_match('/cfg_nok/',
          $var)
      ) {
        if ($getdata = utf8_decode(($cfg['fiwa'] < 650) ? request('GET', "/system/cfgtakeover_edit.lua?sid=$sid&cfg_ok=1") : request('POST', "/data.lua", "xhr=1&sid=$sid&lang=de&no_sidrenew=&page=cfgtakeover_edit"))) {
          ...
          if ($match) { // Decodierte Kennwφrter gefunden
            dbug($match, 4, 'KonfigDeCrypt-Match');
            foreach ($pregimport[$lang] as $key => $var)
              if ($k = preg_array("/^$key$/", $match[2], 1) and preg_match_all($var[2], $match[1][$match[2][$k]], $array))foreach ($array[1] as $k => $v)
            ...
            return errmsg('Keine Entschlόsselte Daten gefunden', __FUNCTION__);
        } else
          return errmsg('Keine Daten erhalten', __FUNCTION__);
      } else
        return errmsg('Entschlόsselungsversuch wurde nicht akzeptiert'
          . (($val = ifset($var, '/<p class="ErrorMsg">(.*?)<\/p>/')) ? (($val[1] == '?1043?')
            ? "\nBitte \"Ausfόhrung bestimmter Einstellungen und Funktionen zusδtzlich bestδtigen\" deaktivieren!" : " - $val[1]") : ''), __FUNCTION__);
      dbug((count($list)) ? floor(count($plain) / (count($list) + count($plain)) * 100) . "% entschlόsselt..." : "100% " . (($buffer) ? "Es konnte" . ((count($buffer) == 1)
        ? " ein Eintrag" : "n " . count($buffer) . " Eintrδge") . " nicht Entschlόsselt werden! " : "") . "- Ersetze " . count($plain) . " entschlόsselte Eintrδge...");
    }
    ...
    if ($binfile) // BinFiles wieder zusammenpacken
      foreach ($binfile as $key => $var)
        $data = preg_replace(
          "/(?<=\*{4} BINFILE:$key\s)[\dA-F\s]*(?=\s\*{4} END OF FILE \*{4})/",
...
function konfig2array($data)
{ // FRITZ!Box-Konfig -> Array
  $config = array();
  if ($data { 0} == '*')
    dbug("Konvertiere Fritz!Konfig...");
  if (
    $data { 0} == '*' and preg_match_all('/^(?:\*{4}\s(.*?)\sCONFIGURATION\sEXPORT|(\w+)=(\S+))\s*$
	|^\*{4}\s(?:CRYPTED)?(CFG|BIN)FILE:(\S+)\s*?\r?\n(.*?)\r?\n^\*{4}\sEND\sOF\sFILE\s\*{4}\s*?$/msx',
      $data,
      $array)
  ) {
...
function showaccessdata($data)
{ // Die Kronjuwelen aus Konfig-Daten heraussuchen
  $text = '';
  $config = array();
  if ($konfig = konfig2array($data)) { // Konfig als Array umwandeln
    ...
    $access = array(
      ...
      'FRITZ!Box-Oberflδche' => array(&$konfig['webui'], '=username,password'),
    ...
    foreach ($access as $key => $var) // Accessliste durcharbeiten
      if (ifset($var[0])) {
        if ($var[1] { 0} == '/') { // Regulδre Ausdrόcke verwenden (Schlόsselname)
          foreach ($var[0] as $k => $v)
            if (preg_match($var[1], $k) and $var[0][$k]) // Schlόssel Suchen und Prόfen
              $config[$key][$k] = $var[0][$k];
        } elseif ($var[1] { 0} == '!' and preg_match($var[1], $var[0], $val) and preg_match_all($var[2], $val[1], $val)) { // Regulδre Ausdrόcke verwenden (Inhalt)
          ...
          foreach ($keys as $k)
            if (ifset($var[0][$k])) // Schlόssel Testen
              $config[$key][$k] = $var[0][$k];
        }
        if (preg_match('/^([-+_])(.+)$/', $var[(isset($var[2])) ? 2 : 1], $keys)) { // Eine Schlόssel-Ebene όberspringen
          ...
          foreach ($var[0] as $k => $v)
            if ((preg_match('/\d+\s*$/', $k, $val) or $keys[1] == '+') and is_array($v)) { // Neue Ebene gefunden
              ...
              foreach ($keys[3] as $val)
                if ($val { 0} == '>' and preg_match('/(\w+)([<>])(\w+)/', $val, $va1) and ifset($var[0][$k][$va1[1]][$va1[3]])) // Mit Regulδre Ausdrόcke noch eine Ebene όberspringen
                  if ($name === false)
                    $name = (string) $var[0][$k][$va1[1]][$va1[3]];
                  else
                    $config[$key][$name][(($va1[2] == '<') ? $va1[1] : $va1[3])] = $var[0][$k][$va1[1]][$va1[3]]; // Den Vorigen Schlόssel verwenden?
                elseif (ifset($var[0][$k][$val])) // Auf der neuen Ebene Prόfen
                  if ($name === false)
    ...
    dbug($config, 4, 'ShowAccessData'); // Alle Fundstόcke ungefiltert sichern
    ...
    if (ifset($config['DECT-eMail'])) // Filter: DECT-eMail
      foreach ($config['DECT-eMail'] as $key => $var) {
        if (ifset($var['server']) and ifset($var['port'])) { // Server & Port zusammenfόhren
    ...
    $a = array('/^\${4}\w+/', '(Verschlόsselt)');
    foreach ($config as $key => $var) // Verschlόsselte Eintrδge umschreiben
      if ($var) {
        if ($val = preg_array($a[0], $var, 3)) { // Array-Schlόssel umbennenen
          foreach ($val as $k => $v) {
            $config[$key][] = $v;
            unset($config[$key][$k]);
          }
...
function getevent($filter = 'aus', $sep = "\t", $sid = 0)
{ // Ereignisse abrufen
  global $cfg;
  ...
  if (
    ifset($data) and preg_match_all(($cfg['fiwa'] < 500) ? '!<p class="log">(\S*)\s*(\S*)\s*(.*?)</p>!'
      : '!<tr><td[^>]*>(?:<div>)?(.*?)(?:</div>)?</td><td[^>]*>(.*?)</td><td[^>]*><a[^>]*>(.*?)</a></td></tr>!',
      $data[1],
      $array)
  ) {
    ...
    $cfg['fiwa'] >= 669 and $data = request('POST-array',
      '/data.lua',
      "xhr=1$sid&lang=de&page=log&xhrId=$filter&no_sidrenew=")
    and preg_match('/"log":\[((?:[^\[\]]++|\[(?1)\])*)\]/',
      $data[1],
      $array)
  )
...
function init($ver)
{ // Fritz!Box Tools Initialisieren
  ...
  if ((float) phpversion() > 4.3 and $ver = ifset($ver, '/^(\w+) ([\d.]+) \(c\) (\d\d)\.(\d\d)\.(\d{4}) by ([\w ]+?) <([.:\/\w]+)>$/')) {
    ...
    define($ver[1], 1); // Feste Kennung fόr Plugins etc.
    ...
    $cfg['ext'] = strtolower(preg_replace('/\W+/', '', pathinfo($script, PATHINFO_EXTENSION))); // Extension fόr Unix/Win32 unterscheidung
    ...
    $cfg['dbcd'] = realpath('.') . '/'; // Current_Dir fόr Debug-Daten
    ...
    if ($_SERVER['argc'] and !preg_match('/cli/', $cfg['php'][2]) and function_exists('header_remove')) { // HTTP-Header lφschen wenn PHP-CGI eingesetzt wird
      header('Content-type:');
    ...
    if (@ini_get('pcre.backtrack_limit') < $cfg['pcre']) // Bug ab PHP 5 beheben (Fόr Groίe RegEx-Ergebnisse)
...
if ($argc and ifset($argv) and init($ver)) { ## CLI-Modus ##
  ...
  if (
    $pset + 1 < $pmax and @preg_match('/^(?!-)	(?:([^:]+):)?	(?:([^@]+)@)?
	([\w.-]+\.[\w.-]+|\[[a-f\d:]+\]|' . strtr(preg_quote(implode("\t",
        array_keys($cfg['preset']))),
        "\t",
        '|') . ')
	(?::(\d{1,5}))?	$/ix',
      $argv[$pset],
      $array)
  ) { // Fritz!Box Anmeldedaten holen
    $cfg['host'] = $array[3];
  ...
  unset($cfg['preset']); // Preset-Daten werden nicht mehr benφtigt!

  # Optionen setzen
  while (
    $argv[$pmax - 1] { 0} == '-' and ($pmax == $argc and !ifset($argv[$pmax - 2], '/^-/')
    ...
    foreach ($array[1] as $key => $var) {
      ...
      if ($var == 's') { // SID
        if (preg_match('/^[\da-f]{16}$/i', $vas, $val) or file_exists($vas) and preg_match('/^[\da-f]{16}$/i', file_get_contents($vas), $val)) // SID direkt όbernehmen
          $cfg['sid'] = $val[0];
      ...
      foreach (array('o' => 'oput', 'un' => 'user', 'pw' => 'pass', 'fb' => 'host', 'pt' => 'port') as $k => $v) // Optionen mit Zwangsparameter ohne Prόfung
        if ($var == $k and $vas)
          $cfg[$v] = $vas;
    }
  }
...
//  $b["backtrace"][] = debug_backtrace();
  ...
  if ($cfg['wrap'] == 'auto') {
    ...
    elseif (
      isset($_SERVER['SystemDrive']) and isset($_SERVER['SystemRoot']) and isset($_SERVER['APPDATA']) and (@exec('mode con',
        $var) or true) // Windows
      and is_array($var) and preg_match_all('/(?:(zeilen|lines)|(spalten|columns)|(code\s?page)):\s*(\S+)/',
        strtolower(implode('',
          $var)),
        $val)
    )
  ...
  if (ifset($cfg['char'], 'auto')) {
    ...
    elseif (
      isset($_SERVER['HOME']) and isset($_SERVER['USER']) and isset($_SERVER['TERM']) and isset($_SERVER['SHELL']) // Unix/Linux/Mac
      and file_exists('/usr/bin/locale') and preg_match('/(utf-?8)|(ansi|iso-?8859-?1|ascii)/i',
        @exec('locale charmap'),
        $var)
    )
  ...
  if (
	...
	|(?<pd>P(ass)?D(ecode)?)|(?<i>I(nfo)?|UpGrade|ug)|(?<fb>FooBar|fb)	)$/ix',
      $argv[$pset],
      $val)
  ) { ## Modes mit und ohne Login ##
    $pset++;
      ...
      elseif ($data = request('GET-array', '/jason_boxinfo.xml') and preg_match_all('!<j:(\w+)>([^<>]+)</j:\1>!m', $data[1], $array)) {
        ...
        foreach ($array[1] as $key => $var)
          $array[0][$key] = str_pad(((isset($jason[$var])) ? $jason[$var][0] : $var) . " ", 20, '.') . " " . ((isset($jason[$var]) and $jason[$var][1])
            ? preg_replace(
              preg_replace('/^((.).+?(?<!\\\\)\2).*\2(\w*)$/', '$1$3', $jason[$var][1]),
              preg_replace('/^(.).+?(?<!\\\\)\1(.*)\1\w*$/', '$2', $jason[$var][1]), $array[2][$key]
            ) : $array[2][$key]);
...
$self gip dns" : ""));
      elseif ($array = getexternalip()) {
        ...
        foreach ($array as $key => $var)
          $val = max($val, strlen($key)); foreach ($array as $key => $var)
          $array[$key] = str_pad($key, $val, " ") . ": $var";
        out(implode("\n", $array));
      } elseif ($var = errmsg(0, 'getexternalip'))
        out($var);
      else
        out("Keine Externe IP-Adresse verfόgbar");
...
UpDate <Check|Force> FB_Tools όber das Internet aktualisieren
...
$self i -d" : ""));
      elseif (
        $pset < $pmax and preg_match('/^(?:(?<php>PHP)|(?<g>G(?:LOBALS)?)|(?<pg>P(?:anGramm|g))|(?<ud>U(?:pdate|d))|(?<e>e|Echo)|(?<wg>wg|W(eb)?Get)|(?<st>st|StrfTime))$/ix',
          $argv[$pset],
          $var)
        or ifset($val['i'],
          '/upgrade|ug/')
        ...
        elseif (ifset($var['pg'])) // Pangramm mit Kitty
          out(wordwrap("Welch fieser Katzentyp quδlt da sόίe Vφgel bloί zum Jux?\n", min((($cfg['wrap']) ? (int) $cfg['wrap'] : 80), 42)) . $cfg['zlib']['inflate'](
            base64_decode(
          ...
          if ($cfg['uplink'] and $fbnet = request('GET-array', $cfg['uplink'][1] . "md5", 0, 0, $cfg['uplink'][0], 80)) { // Update-Check
            if (preg_match("/((\d\d)\.(\d\d)\.(\d{4}))\s[\d:]+\s*\((\w+)\s([\d.]+)\)(?:.*?(\w+)\s\*\w+\.$cfg[ext](?=\s))?/s", $fbnet[1], $up)) {
              ...
              if (intval($up[4] . $up[3] . $up[2]) > $cfg['ver'][8] and floatval($up[6]) > $cfg['ver'][9] or ifset($val[2])) {
                out("Ein Update ist verfόgbar: $up[5] $up[6] vom $up[1]");
                if (!ifset($val[1]) or ifset($val[2])) {
                  ...
                  if (ifset($up[7]) and $up[8] = @request('GET', $cfg['uplink'][1] . "$cfg[ext].gz", 0, 0, $cfg['uplink'][0], 80)) {
                    $rename = preg_replace('/(?=(\.\w+)?$)/', "_{$cfg['ver'][2]}.bak", $script, 1); // Neuer Name fόr alte Version
                    ...
                    out("Automatisches Update ist nicht verfόgbar$manuell");
                }
              } else {
                @touch($script); // Aktuelles Datum setzen
                out("Kein neues Update verfόgbar");
                if (ifset($up[6], $cfg['ver'][9]) and $up[7] != md5_file($script)) // MD5-Check
                  out("Hinweis: $self wurde verδndert");
              }
            } else // fb_tools.md5 nicht verfόgbar
              out("Update-Server sagt NEIN!");
            if (ifset($fbnet['X-Cookie'])) // Coolen Spruch ausgeben
              out("\n" . $fbnet['X-Cookie']);
          } else // Kein Netzwerk/Internet verfόgbar
        ...
        if (isset($cfg['utf8']) and !$cfg['utf8'])
          out("\nHinweis: UTF-8 wird nicht unterstόtzt" . (($cfg['char'] == 'utf7' and preg_match('/linux/i', $cfg['php'][1]))
...
$self 169.254.1.1 rc" : ""));
        ...
        out(($var = forcetermination()) ? "Reconnect ausgefόhrt" : errmsg(0, 'getexternalip'));
...
Folgende Filter sind Mφglich: alle, telefon, internet, usb, wlan, system
...
$self user:pass@169.254.1.1 e logs-%y%m%d.log" : ""));
      elseif ($sid = (ifset($cfg['bsid'])) ? $cfg['bsid'] : login()) { // Einloggen
        ...
        $sep = ($pset < $pmax) ? $argv[$pset++] : " "; // Seperator fόr CSV-Dateien
        $psep = preg_quote($sep, '/');
...
ExPort-DeCrypt  <Datei>  <Kennwort> - Konfig entschlόsseln und exportieren(1,3)
...
File-CalcSum    [Ordner] [File]   - Verδnderter Konfig-Ordner Zusammensetzen(2)
File-JSON       [Datei] [Datei]   - Konfig-Daten in JSON konvertieren(2)
File-DeCrypt    [Datei] [Kennwort] <Datei> - Konfig-Daten entschlόsseln(1,3)
ImPort          [Datei|Ordner] <Kennwort>  - Konfig importieren(1)
ImPort-CalcSum  [Datei|Ordner] <Kennwort>  - Verδnderte Konfig importieren(1)
...
Ab OS 6.69 muss die Sicherheits-Bestδtigungsfunktion deaktiviert werden" . ((preg_match('/[ab]/i', $cfg['help'])) ? "\n
...
$self 169.254.1.1 k ipcs \"FRITZ.Box Fon WLAN 6360 85.04.86_01.01.00_0100.export\"" : ""));
	...
	|(f(?:ile)?)(?:(cs|-calcsum)?|(dc|-de(?:crypt|code))?|(-?json)?)# 8:File 9:CalcSum 10:DeCrypt 11:JSON
        ...
        if (isset($cfg['utf8']) and !$cfg['utf8'] and ($mode[3] or $mode[5] or $mode[10]))
          out("Hinweis: UTF8 wird auf Ihrem System von PHP nicht όnterstόtzt, daher ist die Entschlόsselung Problematisch!");
        if (($mode[2] or $mode[4] or $mode[6])) { // Login Optionen
          if ($sid = (ifset($cfg['bsid'])) ? $cfg['bsid'] : login()) {
            if ($mode[5] or $mode[7]) { // Kennwort-Entschlόsselung
              if ($cfg['fiwa'] > 500) {
                if (!$pass) // Im DeKode-Modus kein leeres Kennwort zulassen
                  $pass = (ifset($cfg['pass'], '/^[ -~]+$/')) ? $cfg['pass'] : 'geheim';
              } else {
                out("Entschlόsselung wird nicht unterstόtzt");
                $mode[5] = $mode[7] = false;
              }
            }
            if ($mode[4]) { // Export
              ...
              if ($mode[5] and $pass and $data = cfgexport('array', $pass) and $data[1]) { // Exportieren mit Entschlόsselten Benutzerdaten
                ...
                out(($var = errmsg(0, 'request')) ? $var : "Keine Konfig erhalten - Mφglichlichweise ist noch die Sicherheits-Bestδtigungsfunktion aktiviert?");
              ...
              if ($data = cfgexport('array', $pass) and $data[1]) { // Konfigdaten holen
                ...
                if ($mod == 1)
                  makedir($file); // Verzeichnis erstellen
                if ($mode[7] and $pass and $data[2] = konfigdecrypt($data[1], $pass, $sid)) // Konfig Entschlόsseln
              ...
              out("$file kann nicht geφffnet werden!");
            if (!ifset($cfg['bsid']))
              logout($sid);
          } else
            out(errmsg(0, 'login'));
        ...
        } elseif ($mode[8] and $mode[10] and !$mode[11] and $pass and is_file($file) and $data = (preg_match($cfg['ptar'], $file)) ? cfgmake(tar2array($file)) : file_read($file)) { // Kennwφrter Entschlόsseln
          if ($sid = (ifset($cfg['bsid'])) ? $cfg['bsid'] : login()) {
            if ($cfg['fiwa'] > 500) { // Entschlόsselung durchfόhren
              if ($data = konfigdecrypt($data, $pass, $sid)) {
                ...
                if ($pset < $pmax and $save = $argv[$pset++]) {
                  if (preg_match('/^(.*?)[\\\\\/]$/', $save, $var)) { // Verzeichnis erstellen
                    ...
                    file_write($save, $data); // Entschlόsselte Konfig sichern
                    $mod = 0;
                  }
                } else
                  $mod = 0;
                out(cfginfo($data, $mod, $save, showaccessdata($data))); // Daten als Text Prδsentieren
              } else {
                out(errmsg(0, 'konfigdecrypt'));
                if (!ifset($pass, '/^[ -~]+$/'))
                  out("Hinweis: Das Konfig-Kennwort enthδlt Sonderzeichen, die bei unterschiedlicher Zeichenkodierung Probleme bereiten kφnnen");
              }
            } else
              out("Entschlόsselung wird nicht unterstόtzt");
            if (!ifset($cfg['bsid']))
          ...
          if (function_exists('json_encode'))
            ...
            out("JSON wird von PHP {$cfg['php'][0]} nicht unterstόtzt");
        else
          out("Parameter-Ressourcen zu Konfig $mode[0] nicht gefunden oder nicht korrekt angegeben");
      } else
        out("Unbekannte Funktionsangabe fόr Konfig");
...
$self 169.254.1.1 d - -pw:geheim" : ""));
      elseif ($sid = (ifset($cfg['bsid'])) ? $cfg['bsid'] : login()) {
        out((dial($argv[$pset++], (($pset < $pmax) ? $argv[$pset++] : false))) ? "Rufnummer wurde gewδhlt" : errmsg(0, 'dial'));
        if (!ifset($cfg['bsid']))
          logout($sid);
      } else
        out(errmsg(0, 'login'));
...
WARNUNG: Es gibt KEINE Prόfung auf Malware!");
      }
...
$self passdecode $example" : ""));
      ...
      out("Mφglichweise ist ein unerwarterer und unbekannter, sowie mysteriφser Fehler aufgetreten :-)");
...
Dial         - Rufnummer wδhlen(2)
Ereignisse   - Systemmeldungen abrufen(2)
GetIP        - Aktuelle externe IPv4-Adresse ausgeben(1)
Info         - FB-Tools/PHP Version, MD5/SHA1 Checksum, Update/Prόfen(3)
Konfig       - Einstellungen Ex/Importieren, Daten entschlόsseln(2,3,4)
LogIn/LogOut - Manuelles Einloggen fόr Scriptdateien(2)$cfg[a1st]
...
(4) Ab OS 6.69 muss die Sicherheits-Bestδtigungsfunktion deaktiviert werden" . ((preg_match('/[ab]/i', $cfg['help'])) ? "\n
Beispiele:
...
$self -h:alles" : "") : "\n\nWeitere Hilfe bekommen Sie mit der -h Option oder mehr Hilfe mit -h:all"));
  ...
  if ($cfg['help']) { // Weitere Hilfe ausgeben
    if (preg_match('/[ao]/i', $cfg['help'])) // Optionen ausgeben
      out("
Alle Optionen:             (Mόssen immer als Letztes angegeben werden!)
         -d .............. Debuginfos
...
Request: -b:[Bytes] ...... Buffergrφίe ($cfg[sbuf])
         -t:[Sekunden] ... TCP/IP Timeout ($cfg[tout])
Login:   -s:[SID|Datei] .. Manuelle SID Angabe (Fόr Scriptdateien)
