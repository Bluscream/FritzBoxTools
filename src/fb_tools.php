...
if(!isset($cfg)) {					// $cfg schon gesetzt?
 $cfg = array(
	...
	'sbuf'	=> 4096,		// TCP/IP Socket-Buffergrφίe
	...
	'dbfn'	=> 'debug#.txt',	// Template fόr Debug-Dateien
...
if(!function_exists('file_put_contents')) {		// http://php.net/file_put_contents
 function file_put_contents($file,$data,$opt=0) {	// $opt ist nicht vollstδndig implemmentiert
...
function ifset(&$x,$y=false) {				// Variabeln prόfen und Optional vergleichen (var,test)
...
function preg_array($x,$y,$z=0,$w=array()) {		// Durchsucht einen Array mit Regulδren Ausdruck (preg,array,mode)
...
function out($str,$mode=0) {				// Textconvertierung vor der ausgabe (mode: 0 -> echo / 1 -> noautolf / 2 -> debug)
 ...
 if($str) {
  ...
  elseif(ifset($cfg['char'],'/^\d+$/') and dechex($cfg['char']) == 539
   and preg_match_all('/([a-z\d])(.)|(.)(..)/i','1I2Z3E4A5S6G7T8B9g0Oa4A4b8B8c<C(e3E3g6G6h#H#i!I!l1L1o0O0q9Q9s5S5t7T7x+X+z2Z2δ4:Δ4:φ0:Φ0:όu:άU:ί55§55',$m)
...
function request($method,$page='/',$body=false,$head=false,$host=false,$port=false) {	// HTTP-Request durchfόhren
 global $cfg;
...
function supportcode($str = false) {			// Supportcode aufschlόsseln
...
function upnprequest($page,$ns,$rq,$exp=false) {	// UPnP Request durchfόhren
 global $cfg;
...
function getupnppath($urn) {				// Helper fόr UPnP-Requests
...
function getexternalip() {				// Externe IPv4-Adresse όber UPnP ermitteln
 global $cfg;
...
function forcetermination() {				// Internetverbindungen όber UPnP neu aufbauen
...
function cfgcalcsum($data) {				// Checksumme fόr die Konfiguration berechnen
...
function cfgimport($file,$pass='',$mode=false,$sid=0) {	// Konfiguration importieren (Wird vermutlich bald όberarbeitet)
 global $cfg;
...
function konfigdecrypt($data,$pass,$sid=0) {		// Konfig-Datei mit Fritz!Box entschlόsseln
 global $cfg;
 ...
 if($sid and preg_match_all('/^\s*([\w-]+)\s*=\s*("?)(\${4}\w+)\2;?\s*$/m',$data,$array) and preg_match('/(?:\s*Password\d*=\${4}\w+)+/',$data,$var)) {
  ...
  $pregimport = array(
   'de' => array('Internetzugangsdaten' => array(1,0,'/^Benutzer:\s(.*?)(?:,\sAnbieter:\s.*?)?$/'), 'Dyn(amic )?DNS' => array(2,1,'/(?<=Domainname:\s|Benutzername:\s)(.*?)(?=,\s|$)/'),
    'PushService' => array(1,3,'/^E-Mail-Empfδnger:\s(.*?)$/'), 'MyFRITZ!' => $dupe[0], 'FRITZ!Box-Benutzer' => $dupe[1]),
   ...
   'fr' => array('Donnιes d\'accθs ΰ Internet' => array(1,0,'/^Utilisateur[\xa0\s]?:[\xa0\s](.*?)(?:,\sProvider:\s.*?)?$/'),
    '(Dyn)?DNS( dynamique)?' => array(2,1,'/(?<=Nom\sde\sdomaine[\xa0\s]:[\xa0\s]|nom\sd\'utilisateur[\xa0\s]:[\xa0\s])(.*?)(?=,\s|$)/'),
    'Service push' => array(1,3,'/^Destinataire\sdu\scourrier\sιlectronique[\xa0\s]?:[\xa0\s](.*?)$/'), 'MyFRITZ!' => $dupe[0], 'Utilisateur de FRITZ!Box' => $dupe[1]),
