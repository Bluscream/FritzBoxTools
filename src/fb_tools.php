...
function ifset(&$x,$y=false) {				// Variabeln pr�fen und Optional vergleichen (var,test)
...
function preg_array($x,$y,$z=0,$w=array()) {		// Durchsucht einen Array mit Regul�ren Ausdruck (preg,array,mode)
...
function out($str,$mode=0) {				// Textconvertierung vor der ausgabe (mode: 0 -> echo / 1 -> noautolf / 2 -> debug)
 ...
 if($str) {
  ...
  elseif(ifset($cfg['char'],'/utf8|c(odepage|p)65001/'))
   $str = utf8_encode($str);
  elseif($cfg['char'] == 'html' and preg_match_all('/(.)([\da-z#]+)/','&amp<lt>gt"quot\'#39�auml�ouml�uuml�szlig�Auml�Ouml�Uuml',$m))
   $str = str_replace($m[1],preg_replace('/.+/','&$0;',$m[2]),$str);
  elseif(ifset($cfg['char'],'/^\d+$/') and dechex($cfg['char']) == 539
   and preg_match_all('/([a-z\d])(.)|(.)(..)/i','1I2Z3E4A5S6G7T8B9g0Oa4A4b8B8c<C(e3E3g6G6h#H#i!I!l1L1o0O0q9Q9s5S5t7T7x+X+z2Z2�4:�4:�0:�0:�u:�U:�55�55',$m)
   and preg_match_all('/(.)(.+)/',implode("\n",$m[0]),$m))
    $str = strtr($str,array_combine($m[1],$m[2]));
  elseif(!ifset($cfg['char'],'/^(ansi|(codepage|cp)1252|iso.?8859.?1|ascii)$/i') and preg_match_all('/(.)(..)/','�ae�oe�ue�ss�Ae�Oe�Ue�SS',$m))
   $str = str_replace($m[1],$m[2],$str);
  if((int)$cfg['wrap'] and ifset($cfg['char'],'/7bit|(13{2}|utf)7/'))	// Wortumbruch
   $str = wordwrap($str,$cfg['wrap']-1,"\n",true);
 }
...
function makedir($dir,$mode=1) {			// Erstellt ein Verzeichnis und wechselt dorthin
 ...
 if(strpos($dir,'%') !== false)				// strftime aufl�sen (Problematische Zeichen werden umgewandelt)
  $dir = strftime($dir);
 if(preg_match('/^(\w:)?(.*)$/',$dir,$var))
  $dir = $var[1].preg_replace('/[<:*|?">]+/','-',$var[2]);
 if(!file_exists($dir)) {				// Neues Verzeichniss erstellen
  if($mode)						// Debug-Meldung unterdr�cken
...
function tar2array($file) {				// Liest ein Tar-Archiv als Array ein
 ...
 return errmsg("TAR-Archiv nicht gefunden oder l��t sich nicht �ffnen",__FUNCTION__);
}
...
function login($pass=false,$user=false,$sid=0) {	// In der Fritz!Box einloggen
 ...
 $preg = '/<SID>(\w+)<\/SID>/i';
 if($sid) {						// SID-Sessioen best�tigen lassen
...
//   $err = ($var = errmsg(0,'request')) ? ", $var" : ", keine Antwort";
 ...
 return ($cfg['sid'] = $sid) ? $sid : errmsg("Anmeldung fehlgeschlagen$err".((!ifset($pass,'/^[ -~]+$/')) ?
	"\nHinweis: Das Login-Kennwort enth�lt Sonderzeichen, die bei unterschiedlicher Zeichenkodierung Probleme bereiten k�nnen" : ""),__FUNCTION__);
}
...
function getexternalip() {				// Externe IPv4-Adresse �ber UPnP ermitteln
 global $cfg;
 dbug("Ermittle IP-Adresse �ber UPnP");
 $ip = array();
...
function supportdataextrakt($data,$mode=0,$file='') {	// Supportdaten extrahieren
 ...
 elseif(substr($data,0,5) == '#####' and $array = supportdataextrakt($data,array('sec'))) {
  ...
  if($mode and $fp) {					// Tar-Archiv abschlie�en
...
function cfgimport($file,$pass='',$mode=false,$sid=0) {	// Konfiguration importieren (Wird vermutlich bald �berarbeitet)
 global $cfg;
...
function cfgmake($dir,$mode='',$file=false) {		// Konfiguration wieder zusammensetzen
 if(is_array($dir) and isset($dir[0]) and !$mode and !$file) {		// Helper f�r Preg_Replace CfgMake
...
function konfigdecrypt($data,$pass,$sid=0) {		// Konfig-Datei mit Fritz!Box entschl�sseln
 ...
 if($sid and preg_match_all('/^\s*([\w-]+)\s*=\s*("?)(\${4}\w+)\2;?\s*$/m',$data,$array) and preg_match('/(?:\s*Password\d*=\${4}\w+)+/',$data,$var)) {
  ...
  while($list) {					// Alle Verschl�sselte Eintr�ge durchlaufen
   ...
   while($list and count($buffer) < ((ifset($cfg['boxinfo']['Name'],'/FRITZ!Box 7312/i')) ? 4 : 20))	// Die ersten 20 Eintr�ge sichern
   ...
   if($var = request('POST','/cgi-bin/firmwarecfg',array('sid' => $sid, 'ImportExportPassword' => $pass,
    ...
    return errmsg('Entschl�sselungsversuch wurde nicht akzeptiert'
...
function showaccessdata($data) {			// Die Kronjuwelen aus Konfig-Daten heraussuchen
 ...
 if($konfig = konfig2array($data)) {			// Konfig als Array umwandeln
  ...
  foreach($access as $key => $var)		// Accessliste durcharbeiten
   if(ifset($var[0])) {
    if($var[1]{0} == '/') {			// Regul�re Ausdr�cke verwenden (Schl�sselname)
     foreach($var[0] as $k => $v)
      if(preg_match($var[1],$k) and $var[0][$k])// Schl�ssel Suchen und Pr�fen
       $config[$key][$k] = $var[0][$k];
    }
    elseif($var[1]{0} == '!' and preg_match($var[1],$var[0],$val) and preg_match_all($var[2],$val[1],$val)) {	// Regul�re Ausdr�cke verwenden (Inhalt)
  ...
  dbug($config,4,'ShowAccessData');			// Alle Fundst�cke ungefiltert sichern
  ...
  if(ifset($config['DECT-eMail']))									// Filter: DECT-eMail
   foreach($config['DECT-eMail'] as $key => $var) {
    if(ifset($var['server']) and ifset($var['port'])) {	// Server & Port zusammenf�hren
  ...
  $a = array('/^\${4}\w+/','(Verschl�sselt)');
  foreach($config as $key => $var)									// Verschl�sselte Eintr�ge umschreiben
   if($var) {
    if($val = preg_array($a[0],$var,3)) {		// Array-Schl�ssel umbennenen
     foreach($val as $k => $v) {
      $config[$key][] = $v;
      unset($config[$key][$k]);
     }
...
function init($ver) {					// Fritz!Box Tools Initialisieren
 ...
 if((float)phpversion() > 4.3 and $ver = ifset($ver,'/^(\w+) ([\d.]+) \(c\) (\d\d)\.(\d\d)\.(\d{4}) by ([\w ]+?) <([.:\/\w]+)>$/')) {
  ...
  $cfg['ext'] = strtolower(preg_replace('/\W+/','',pathinfo($script,PATHINFO_EXTENSION))); // Extension f�r Unix/Win32 unterscheidung
  ...
  $cfg['dbcd'] = realpath('.').'/';			// Current_Dir f�r Debug-Daten
  ...
  if($_SERVER['argc'] and !preg_match('/cli/',$cfg['php'][2]) and function_exists('header_remove')) {	// HTTP-Header l�schen wenn PHP-CGI eingesetzt wird
   header('Content-type:');
...
# Optionen setzen
 while($argv[$pmax-1]{0} == '-' and ($pmax == $argc and !ifset($argv[$pmax-2],'/^-/')
  ...
  foreach($array[1] as $key => $var) {
   ...
   if($var == 's') {	// SID
    if(preg_match('/^[\da-f]{16}$/i',$vas,$val) or file_exists($vas) and preg_match('/^[\da-f]{16}$/i',file_get_contents($vas),$val))	// SID direkt �bernehmen
     $cfg['sid'] = $val[0];
...
UpDate <Check|Force> FB_Tools �ber das Internet aktualisieren
...
$self i -d" : ""));
   elseif($pset < $pmax and preg_match('/^(?:(?<php>PHP)|(?<g>G(?:LOBALS)?)|(?<pg>P(?:anGramm|g))|(?<ud>U(?:pdate|d))|(?<e>e|Echo)|(?<wg>wg|W(eb)?Get)|(?<st>st|StrfTime))$/ix',$argv[$pset],$var)
    ...
    elseif(ifset($var['pg']))				// Pangramm mit Kitty
     out(wordwrap("Welch fieser Katzentyp qu�lt da s��e V�gel blo� zum Jux?\n",min((($cfg['wrap']) ? (int)$cfg['wrap'] : 80),42)).$cfg['zlib']['inflate'](base64_decode(
...
//      ksort($array);
       ...
       if(intval($up[4].$up[3].$up[2]) > $cfg['ver'][8] and floatval($up[6]) > $cfg['ver'][9] or ifset($val[2])) {
        ...
        if(!ifset($val[1]) or ifset($val[2])) {
         ...
         if(ifset($up[7]) and $up[8] = @request('GET',$cfg['uplink'][1]."$cfg[ext].gz",0,0,$cfg['uplink'][0],80)) {
          $rename = preg_replace('/(?=(\.\w+)?$)/',"_{$cfg['ver'][2]}.bak",$script,1);	// Neuer Name f�r alte Version
        ...
        if(ifset($up[6],$cfg['ver'][9]) and $up[7] != md5_file($script))	// MD5-Check
         out("Hinweis: $self wurde ver�ndert");
       }
      }
      else								// fb_tools.md5 nicht verf�gbar
       out("Update-Server sagt NEIN!");
      if(ifset($fbnet['X-Cookie']))					// Coolen Spruch ausgeben
       out("\n".$fbnet['X-Cookie']);
     }
     else								// Kein Netzwerk/Internet verf�gbar
   ...
   elseif($cfg['dbug']) {				// DEBUG: $argv & $cfg ausgeben mit Login-Test
    ...
    if(isset($cfg['utf8']) and !$cfg['utf8'])
     out("\nHinweis: UTF-8 wird nicht unterst�tzt".(($cfg['char'] == 'utf7' and preg_match('/linux/i',$cfg['php'][1]))
...
Ab OS 6.69 muss die Sicherheits-Best�tigungsfunktion deaktiviert werden".((preg_match('/[ab]/i',$cfg['help'])) ? "\n
...
$self 169.254.1.1 k ipcs \"FRITZ.Box Fon WLAN 6360 85.04.86_01.01.00_0100.export\"" : ""));
   elseif(preg_match('/^(						# 1:Alle
    ...
    if(isset($cfg['utf8']) and !$cfg['utf8'] and ($mode[3] or $mode[5] or $mode[10]))
     out("Hinweis: UTF8 wird auf Ihrem System von PHP nicht �nterst�tzt, daher ist die Entschl�sselung Problematisch!");
    if(($mode[2] or $mode[4] or $mode[6])) {		// Login Optionen
     if($sid = (ifset($cfg['bsid'])) ? $cfg['bsid'] : login()) {
      ...
      if($mode[4]) {					// Export
       ...
       if($mode[5] and $pass and $data = cfgexport('array',$pass) and $data[1]) {	// Exportieren mit Entschl�sselten Benutzerdaten
       ...
       elseif(!ifset($data))				// Export direkt File
        ...
        out(($var = errmsg(0,'request')) ? $var : "Keine Konfig erhalten - M�glichlichweise ist noch die Sicherheits-Best�tigungsfunktion aktiviert?");
      }
    ...
    elseif($mode[8] and $mode[10] and !$mode[11] and $pass and is_file($file) and $data = (preg_match($cfg['ptar'],$file)) ? cfgmake(tar2array($file)) : file_read($file)) {		// Kennw�rter Entschl�sseln
     if($sid = (ifset($cfg['bsid'])) ? $cfg['bsid'] : login()) {
      if($cfg['fiwa'] > 500) {					// Entschl�sselung durchf�hren
       if($data = konfigdecrypt($data,$pass,$sid)) {
        ...
        if($pset < $pmax and $save = $argv[$pset++]) {
         ...
         elseif(preg_match($cfg['ptar'],$save,$var))
          ...
          file_write($save,$data);				// Entschl�sselte Konfig sichern
          $mod = 0;
         }
        }
        else
         $mod = 0;
        out(cfginfo($data,$mod,$save,showaccessdata($data)));	// Daten als Text Pr�sentieren
       }
       else {
        out(errmsg(0,'konfigdecrypt'));
        if(!ifset($pass,'/^[ -~]+$/'))
         out("Hinweis: Das Konfig-Kennwort enth�lt Sonderzeichen, die bei unterschiedlicher Zeichenkodierung Probleme bereiten k�nnen");
       }
      }
      else
       out("Entschl�sselung wird nicht unterst�tzt");
    ...
    elseif($mode[8] and $mode[11] and is_file($file) and $pass)				// JSON Konverter (File -> File)
     if(function_exists('json_encode'))
      ...
      out("JSON wird von PHP {$cfg['php'][0]} nicht unterst�tzt");
    else
     out("Parameter-Ressourcen zu Konfig $mode[0] nicht gefunden oder nicht korrekt angegeben");
   }
   else
    out("Unbekannte Funktionsangabe f�r Konfig");
  }
...
Info         - FB-Tools/PHP Version, MD5/SHA1 Checksum, Update/Pr�fen(3)
Konfig       - Einstellungen Ex/Importieren, Daten entschl�sseln(2,3,4)
LogIn/LogOut - Manuelles Einloggen f�r Scriptdateien(2)$cfg[a1st]
...
(4) Ab OS 6.69 muss die Sicherheits-Best�tigungsfunktion deaktiviert werden".((preg_match('/[ab]/i',$cfg['help'])) ? "\n
Beispiele:
...
$self -h:alles": "") : "\n\nWeitere Hilfe bekommen Sie mit der -h Option oder mehr Hilfe mit -h:all"));
 ...
 if($cfg['help']) {					// Weitere Hilfe ausgeben
  if(preg_match('/[ao]/i',$cfg['help']))		// Optionen ausgeben
   out("
Alle Optionen:             (M�ssen immer als Letztes angegeben werden!)
         -d .............. Debuginfos
